...
`OpIDSum` is a compact representation of an operator sum whose terms are stored as
fixed-width tuples of `OpID` values together with scalar coefficients.
...
Fields:
- `terms`: A 2D view of the terms comprising the operator.
...
`os.abs_tol` are skipped. Importantly, the sorting is stable.
"""
