...
- `qn_flux`: quantum-number flux carried by the operator, or `QN()` when the
  ITensor operator reports no flux.
"""
struct OpInfo
  name::String
  matrix::Matrix
...
Construct an `OpInfo` by evaluating `local_op` on `site`.

This caches the operator name, dense matrix, fermionic parity, and QN flux used
later by `OpIDSum` and MPO graph construction.
...
`OpCacheVec` is indexed as `op_cache_vec[n][op_id]`, where `n` is a site
number and `op_id` is the operator id stored in an `OpID`. By convention,
operator id `1` is the identity on every site.
"""
...
The number of site caches must match `sites`, and the first cached operator for
every site must be the identity. The input cache is returned unchanged.
"""
...
The returned cache preserves this ordering, so ids in `OpID` values refer to
positions in the corresponding site list.
"""
...
is multiplied by `-1`. Non-fermionic swaps do not affect the sign. Operators on
the same site keep their original relative order.
"""
...
Each stored term has a scalar coefficient and up to `N` non-identity local
operators encoded as `OpID{Ti}` values. Unused slots are padded with
`zero(OpID{Ti})`. The operator ids are interpreted through `op_cache_vec`, and
terms are stored in increasing site order after insertion.
...
- `modify!`: callback applied to the nonzero part of a term before storing its
  scalar.
"""
...
The optional `modify!` callback receives a view of the non-identity `OpID`
entries for each added term and returns a scalar factor to multiply into that
term coefficient. Terms with `abs(scalar) <= abs_tol` are ignored before any
storage slot is claimed.

Type Parameters:
- `N`: The maximum number of onsite operators per term, i.e. the weight of the
  operator.
- `C`: The scalar coefficient type.
- `Ti`: The integer type used to identify both an operator and the sites.
"""
function OpIDSum{N,C,Ti}(
  max_terms::Int, op_cache_vec::OpCacheVec, modify!::Function; abs_tol::Real=0
...
`os.abs_tol` are skipped. Importantly, the sorting is stable. This does not
...
added to the resulting `OpIDSum` using those cached identifiers. The storage
width is the maximum number of local operators in any input term, with a minimum
width of two to support the reinterpreted fixed-width layout.
"""
