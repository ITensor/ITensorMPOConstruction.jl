...
The answer is of course system dependent, and we recommend trying both out to see which performs better for your specific system. Below we list the pros and cons of each algorithm, in case you want to build an intuition for why one works better than the other.
...
* Uses more memory and slower than the vertex cover algorithm for unstructured problems.
...
So, if $\operatorname{rank}(\gamma) \ll \operatorname{structural\,rank}(\gamma)$, then the problem is structured and the QR decomposition algorithm will be by far the superior option. If instead, $\operatorname{rank}(\gamma) \approx \operatorname{structural\,rank}(\gamma)$, then the problem is unstructured and the minimum vertex cover will perform well.