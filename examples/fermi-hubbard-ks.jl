...
# ## Results
# We constructed the 1D momentum space Fermi-Hubbard Hamiltonian using ITensorMPS and ITensorMPOConstruction with the QR algorithm. For even $N$, the Hamiltonian can be represented exactly as an MPO of bond dimension ``10 N - 4``, and both methods achieve this minimum bond dimension. However, ITensorMPOConstruction is also not only able to construct this particular MPO much faster, but the sparsity of the resulting MPO is much higher. This is particularly interesting because the MPO from ITensorMPS was constructed with `splitblocks=true`, whereas for ITensorMPOConstruction we used `splitblocks=false`. The reason for this drastic difference in sparsity is that the momentum space Fermi-Hubbard Hamiltonian conserves momentum, and even though we didn't enforce this symmetry, ITensorMPOConstruction was able to exploit it via the connected components sub-routine. These timings were taken with `julia -t8 --gcthreads=8,1` on a 2021 MacBook Pro with the M1 Max CPU and 32GB of memory. In order to take advantage of the `OpIDSum` machinery, the ITensorMPOConstruction data is from [`fermi-hubbard-tc.jl`](./fermi-hubbard-tc.md) with equivalent settings.
# 
